"""Main ACP client for programmatic workflow execution."""
...
class ACP:
    ...
    This class provides a Python API for loading ACP specifications and
    executing workflows. It wraps the acp-compiler and acp-runtime packages.
    ...
    Example:
        ...
        acp = ACP.from_path("path/to/project/", variables={"api_key": "..."})

        # Run a workflow
        result = await acp.run_workflow("my_workflow", input_data={"question": "Hello"})
        print(result.output)
        ```
    """
    ...
    def __init__(
        ...
        """Initialize ACP client with a compiled spec.

        Use `ACP.from_path()` to create an instance from .acp files.

        Args:
            spec: Compiled ACP specification
    ...
    def from_path(
        ...
        If path is a directory, all .acp files in it are discovered and compiled
        ...
        Returns:
            ACP instance ready to run workflows
        ...
        Example:
            ```python
            # Load from directory
            acp = ACP.from_path("./my-project/", variables={"api_key": "sk-..."})

            # Load from single file
            acp = ACP.from_path("agent.acp", variables={"api_key": "sk-..."})
            ```
        """
        path = Path(path)
    ...
    def __repr__(self) -> str:
        ...
        return f"<ACP workflows={workflow_count} agents={agent_count}>"
