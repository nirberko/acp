"""Module loader for ACP module system.

Handles loading, parsing, and parameter injection for ACP modules.
"""
...
class LoadedModule:
    ...
    def get_exported_resources(self) -> dict[str, list[str]]:
        ...
        """
        return {
            "providers": [p.full_name for p in self.acp_file.providers],
            "servers": [s.name for s in self.acp_file.servers],
            "capabilities": [c.name for c in self.acp_file.capabilities],
            "policies": [p.name for p in self.acp_file.policies],
            "models": [m.name for m in self.acp_file.models],
            "agents": [a.name for a in self.acp_file.agents],
            "workflows": [w.name for w in self.acp_file.workflows],
        }


class ModuleLoader:
    """Loads and parses ACP modules."""
    ...
    def load_module(
        ...
        Args:
            module_block: Parsed module block from ACP file
