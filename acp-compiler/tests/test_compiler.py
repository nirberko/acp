...
class TestCompileSpec:
    """Tests for compile_acp function."""
...
"""
        ir = compile_acp(acp_content, check_env=False, resolve_credentials=False)
...
"""
        ir = compile_acp(acp_content, check_env=True, resolve_credentials=True)
...
class TestCompileSpecFile:
    """Tests for compile_acp_file function."""

    def test_compile_existing_file(self, monkeypatch):
        """Test compiling an existing ACP file."""
        monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
...
"""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".acp", delete=False) as f:
            ...
            ir = compile_acp_file(f.name, check_env=False, resolve_credentials=False)
...
"""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".acp", delete=False) as f:
            ...
            ir = compile_acp_file(Path(f.name), check_env=False)
...
class TestValidateSpecFile:
    """Tests for validate_acp_file function."""

    def test_validate_valid_file(self):
        """Test validating a valid ACP file."""
        acp_content = """
acp {
  version = "0.1"
  project = "valid-test"
...
"""
    ...
    def test_validate_nonexistent_file(self):
        """Test that non-existent file raises CompilationError."""
        with pytest.raises(CompilationError) as exc_info:
            validate_acp_file("/nonexistent/path.acp")
        assert "File not found" in str(exc_info.value) or "Parse error" in str(exc_info.value)
